...
public class AuthServiceImpl implements AuthService {
    ...
    public MessageResponse verifyEmail(String token) {
        ...
        if (user.getVerificationToken() == null || user.getVerificationTokenExpiry().isBefore(Instant.now())){
    ...
    public MessageResponse forgotPassword(String email) {
        ...
        user.setPasswordResetTokenExpiry(Instant.now().plusSeconds(600));
        userRepository.save(user);
    ...
    public MessageResponse resetPassword(String token, String newPassword) {
        User user = userRepository.findByPasswordResetToken(token)
                .orElseThrow(()->new InvalidTokenException("Invalid or expired rest token"));
